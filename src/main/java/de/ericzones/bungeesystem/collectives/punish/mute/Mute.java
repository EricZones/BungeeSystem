...
public class Mute implements IPunish {

    private final UUID uuid, creator;
    private final MuteReason reason;
    private final Long expiry, creationTime;

    public Mute(UUID uuid, MuteReason reason, int previousMutesCount, UUID creator) {
        this.uuid = uuid;
        this.creator = creator;
        this.reason = reason;
        this.creationTime = System.currentTimeMillis();
        if(reason.getDuration() != -1)
            this.expiry = System.currentTimeMillis() + (reason.getDuration()*previousMutesCount)*1000;
        else
            this.expiry = -1L;
    }

    public Mute(UUID uuid, MuteReason reason, Long expiry, UUID creator, Long creationTime) {
        this.uuid = uuid;
        this.creator = creator;
        this.reason = reason;
        this.expiry = expiry;
